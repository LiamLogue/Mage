...
namespace Mage
{
    class Spikes
    {
        ...
        public Spikes(Texture2D pTexture)
        {
            //Set the texture and make our health totem invisible
            this.spikesTexture = pTexture;
            isVisible = false;
        }
        #endregion
        ...
        {
            //Update the rectangle for the spikes
            rectangle = new Rectangle((int)pPlayerPos.X, (int)pPlayerPos.Y, spikesTexture.Width, spikesTexture.Height);
            borderRectangle = new Rectangle((int)pPlayerPos.X - 60, (int)pPlayerPos.Y - 55, spikesTexture.Width * 8, spikesTexture.Height * 8);

            //Draw the spikes
            spriteBatch.Draw(spikesTexture, pPlayerPos, null, Color.White, 0.0f, origin, 1.0f, SpriteEffects.None, 0.0f);
        }
        #endregion
